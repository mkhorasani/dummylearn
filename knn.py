...
def knn_main(engine):
    ...
    update_row('knn1',datetime.now().strftime("%H:%M:%S %d/%m/%Y"),session_id,engine)

    if status == True:
        update_row('data1_rows',len(df),session_id,engine)
        update_row('knn2',datetime.now().strftime("%H:%M:%S %d/%m/%Y"),session_id,engine)
        col_names = list(df)
        ...
        try:
            ...
            update_row('knn3',datetime.now().strftime("%H:%M:%S %d/%m/%Y"),session_id,engine)
            ...
            if status_test == True:
                try:
                    ...
                    update_row('knn4',datetime.now().strftime("%H:%M:%S %d/%m/%Y"),session_id,engine)
                except:
    ...
    if st.sidebar.button('Download sample dataset'):
        url = 'https://raw.githubusercontent.com/mkhorasani/dummylearn/main/Sample%20datasets/data2.csv?token=AJIVK65CQ2IQ7DDIEZIAAODBE24VM'
        csv = pd.read_csv(url)
