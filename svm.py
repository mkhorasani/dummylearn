...
def svm_main(engine):
    ...
    update_row('svm1',datetime.now().strftime("%H:%M:%S %d/%m/%Y"),session_id,engine)

    if status == True:
        update_row('data1_rows',len(df),session_id,engine)
        update_row('svm2',datetime.now().strftime("%H:%M:%S %d/%m/%Y"),session_id,engine)
        col_names = list(df)
        ...
        try:
            ...
            hyperparameters = {'C':[C], 'kernel':[kernel], 'degree':[degree], 'gamma':[gamma],
            'coef0':[coef0], 'shrinking':[shrinking],
            'probability':[probability], 'tol':[tol], 'class_weight':[class_weight],
            'verbose':[verbose], 'max_iter':[max_iter], 'decision_function_shape':[decision_function_shape],
            'break_ties':[break_ties], 'random_state':[random_state]}

            st.subheader('Download hyperparameters')
            st.markdown(download(pd.DataFrame(hyperparameters),'DummyLearn.com - Support Vector Machine Classifier - Hyperparameters'), unsafe_allow_html=True)

            st.sidebar.subheader('Test Dataset')
            status_test, df_test = file_upload('Please upload a test dataset')
            update_row('svm3',datetime.now().strftime("%H:%M:%S %d/%m/%Y"),session_id,engine)
            ...
            if status_test == True:
                try:
                    ...
                    st.markdown(download(X_pred,'DummyLearn.com - Support Vector Machine Classifier - Predicted Labels'), unsafe_allow_html=True)
                    update_row('svm4',datetime.now().strftime("%H:%M:%S %d/%m/%Y"),session_id,engine)
                except:
